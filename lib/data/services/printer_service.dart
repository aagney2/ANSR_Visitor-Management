...
class PrinterService {
  ...
  Future<List<BrotherDevice>> discoverPrinters({int timeoutSeconds = 5}) async {
    try {
  ...
  /// Save a discovered BrotherDevice as the selected printer.
  Future<void> saveBrotherDevice(BrotherDevice device) async {
  ...
  /// Print a badge image (PNG bytes) using the Brother SDK.
  /// Tries multiple label sizes to find one that matches.
  Future<void> printBadge(Uint8List imageBytes) async {
    ...
    try {
      // Convert PNG badge to a proper PDF
      ...
      // QL-820NWB (2-color printer) uses RB variant for all 62mm rolls
      final labelSizesToTry = <BrotherLabelSize?>[
        ...
        null, // auto-detect as last resort
      ];
      ...
      for (final labelSize in labelSizesToTry) {
        try {
          ...
          if (e.code != 'BRPrintErrorCodeSetLabelSizeError') {
            // Different error — don't keep trying label sizes
            try { await pdfFile.delete(); } catch (_) {}
            rethrow;
          }
